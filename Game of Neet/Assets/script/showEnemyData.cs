...
namespace battle
{
    ...
    public class showEnemyData : MonoBehaviour
    {
        ...
        IEnumerator StartChat(){
            yield return null;
            EventLog.text = "";
            board.transform.localScale = new Vector3(1, 1.5f, 1);
            board.transform.position = new Vector3(0, -1, 0);
            firstButtonSet.SetActive(true);
            returnB.SetActive(true);
            text1.rectTransform.localPosition = new Vector3(0, 65, 0);
            text1.text = Dname[DungeonManager.Instance.DungeonENum]+"[進行度" + currentStg + "/" + TempS[0].DungeonLevel + "]";
            text2.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
        ...
        IEnumerator Engage()
        {
            ...
            board.transform.position = new Vector3(0, -2.4f, 0);
        ...
        public void itemButton()
        {
            ...
            if (TempS[0].ItemStgNum >0)
            {
                ...
                for (int i = 0; i < loadeditem; i++)
                {
                    ...
                    LoadStorage(new Vector3(-110 + (Spacing * 55), 120 - ((i / 5) * 80), 0), CIlist[0].ItemName[Ditem[i][1]], "X" + CIlist[Stages + 3].ItemNum[Ditem[i][1]].ToString(), ButtonOnClick, ItemCanvas.transform);
            ...
            BItem = 0;
                for (int i = 0; i < 5; i++)
                {
                    ...
                    for(int j= 0; j < CIlist.ItemNum.Length - 1; j++)
                    {
                        ...
                        {
                            LoadStorage(new Vector3(-65 + spacingCount*(65), 250 - (BItem / 3) * 80, 0),CIlist.ItemName[j], "X" + CIlist.ItemNum[j].ToString(), ButtonOnClick,BagCanvas.transform);
                            spacingCount++;
